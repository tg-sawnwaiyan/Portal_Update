    ...
    methods: {
        updateAds() {               
            this.$swal({
                ...
                console.log('yyyy');
                this.$loading(true);
                this.axios.post(`/api/news/update/${this.$route.params.id}`, newsData).then((response) => {
                    this.$loading(false);                           
                    this.$swal({
                        position: 'top-end',
                        type: 'success',
                        text: 'お知らせ・ニュースを更新しました。',                             
        ...
        clickValidation() {
            ...
            {    
                this.errors.status = "表示されたいタイプは必須です。";
            }
            ...
            {    
                this.errors.description = "内容は必須です。";
            }
