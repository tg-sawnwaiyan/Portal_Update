...
<layout>
<div>
  <div class="col-md-12 hospital-map-search">
    <h5 class="font-weight-bold"><i class="fas fa-map color-2980b9"></i>&nbsp;地図検索 
        ...
        <div>
          ...
          <div class="row map-wrap">
            <div class="col-lg-5 col-md-12 col-sm-12 float-left map-leftwrapper">
              ...
              <div class="wrap">
                <div class="search">
                  <input type="text" id="search-free-word" class="searchTerm term-border" placeholder="地名、病院名、診療科目などを入力">
                  <button type="submit" class="searchButton search-btn" @click="searchfreeword">
          ...
          <div id="scroll-responsive">
            <div id="hos_search" class="col-12 hospitalselect pad-free m-b-10">
              <div>
                <h5 class="profile_header m-t-10 profile-header-border">現在の検索条件</h5>
                <table class="table table-bordered col-12 border-none">
                  <tbody>
                    <tr>
                            ...
                            <div class="hospital-subject"  v-for="(subject,index) in subjects.slice((i*3),((i*3)+3))"  :key="index" v-bind:class="{ lastblock: i==array_len-1 }">
                              ...
                              <div class="row col-md-12">
                                <div class="col-lg-6 col-md-12 pad-free" v-for="ch in subject.child" :key="ch.id+1">
                                  <label class="form-check-label control control--checkbox padding-left-5">
                ...
                <div class="col-12">
                  <div class="row">
                    <div id="job_detail" class="col-md-12 col-sm-12 pad-free offset margin-top-20" v-for="(hos,index) in displayItems" :key="hos.hos_id">
                      <div class="hos-content">
                          ...
                          <div class="row pad-free">
                            ...
                            <div class="col-md-2 col-sm-12">
                              <p class="float-right pc-768">                                           
                                <span class="btn fav-profile fav-item fav-color display-block" v-if="hos.fav_check == '' && !$auth.check()" :class="'view_pro_id'+hos.nursing_id" @click="favAddFun('add',hos.hos_id,index);"><i class="fas fa-plus-square color-c40000"></i>&nbsp; お気に入りに追加</span>
                                <span class="btn fav-profile fav-item fav-color color-c40000 display-block" v-if="hos.fav_check == 'check' && !$auth.check()" :class="'done_pro_id'+hos.nursing_id" @click="favAddFun('remove',hos.hos_id,index);"><i class="fas fa-check-double color-c40000"></i>&nbsp; 追加済み</span>
                              </p>
                            </div>
                          </div>
                        ...
                        <div class="hos-body row clearfix">
                          ...
                          <div class="col-lg-4 col-md-6 job-box">
                            <table  class="table table-bordered table-sm">                                               
                              <tr>
                                <td class="width-34"><span class="job_ico"><i class="fa fa-envelope"></i></span>メールアドレス</td>
                                <td class="text-break">{{hos.email}}</td>
                              </tr>
                              <tr>
                                <td class="width-30"><span class="job_ico"><i class="fa fa-map-marker-alt"></i></span>住所</td>
                            ...
                            <table class="table table-bordered consultTable">                                                
                              <tbody>
                                ...
                                <tr v-for="(time,index) in timetable" :key="index+'-'+time.id+'-'+hos.hos_id" class="text-center">
                                  <th class="second-hos-row width-8" v-if="(hos.hos_id == time.profile_id && time.part == 'am' )">午前</th>
                                  <th class="second-hos-row width-8" v-if="(hos.hos_id == time.profile_id && time.part == 'pm' )">午後</th>
                                  <th class="width-10" v-if="hos.hos_id == time.profile_id">{{time.mon}}</th>
                                  <th class="width-10" v-if="hos.hos_id == time.profile_id">{{time.tue}}</th>
                                  <th class="width-10" v-if="hos.hos_id == time.profile_id">{{time.wed}}</th>
                                  <th class="width-10" v-if="hos.hos_id == time.profile_id">{{time.thu}}</th>
                                  <th class="width-10" v-if="hos.hos_id == time.profile_id">{{time.fri}}</th>
                                  <th class="width-10" v-if="hos.hos_id == time.profile_id">{{time.sat}}</th>
                                  <th class="width-10" v-if="hos.hos_id == time.profile_id">{{time.sun}}</th>
                                </tr>
                              </tbody>
                            </table>
                        ...
                        <div class="row m-t-10">
                          ...
                          <div class="col-md-6 col-sm-12">
                              <p class="sp-768">
                              <span class="btn fav-profile fav-item fav-color display-block" v-if="hos.fav_check == '' && !$auth.check()" :class="'view_pro_id'+hos.nursing_id" @click="favAddFun('add',hos.hos_id,index);"><i class="fas fa-plus-square color-c40000"></i>&nbsp; お気に入りに追加</span>
                              <span class="btn fav-profile fav-item fav-color display-block color-aaa" v-if="hos.fav_check == 'check' && !$auth.check()" :class="'done_pro_id'+hos.nursing_id" @click="favAddFun('remove',hos.hos_id,index);"><i class="fas fa-check-double color-c40000"></i>&nbsp; 追加済み</span>
                            </p>
                          </div>
                        </div>
